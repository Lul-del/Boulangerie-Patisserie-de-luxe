      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <Reveal className="text-center mb-5 sm:mb-8">
          ...
          <h2 className="font-poppins text-xl xs:text-2xl sm:text-3xl md:text-4xl font-bold text-white mt-2 mb-3 sm:mb-4">
            Ce que disent <span className="text-gold-400">nos clients</span>
          </h2>
        </Reveal>
              ...
              <div className="flex gap-2">
                {testimonials.map((_, i) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all duration-300 ${i === current ? "w-8 bg-gold-500" : "w-2 bg-white/30 hover:bg-white/50"}`}
                  />
                ))}
              </div>
              <button
